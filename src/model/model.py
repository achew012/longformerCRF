...
class Transformer_CRF(nn.Module):
    def __init__(self, num_labels, start_label_id):
        ...
        self.transitions = nn.Parameter(
            torch.randn(self.num_labels, self.num_labels), requires_grad=True
        )
        ...
        self.path = nn.Parameter(
            torch.zeros(1, 1, dtype=torch.long), requires_grad=False
        )
    ...
    def log_sum_exp_batch(log_Tensor, axis=-1):
        ...
        sum_score = torch.exp(
            log_Tensor - torch.max(log_Tensor, axis)[0].view(log_Tensor.shape[0], -1, 1)
        ).sum(axis)
        return torch.max(log_Tensor, axis)[0] + torch.log(sum_score)

    def reset_layers(self):
        self.log_alpha = self.log_alpha.fill_(0.0)
        self.score = self.score.fill_(0.0)
        self.log_delta = self.log_delta.fill_(0.0)
        self.psi = self.psi.fill_(0.0)
    ...
    def _forward_alg(self, feats):
        ...
        log_alpha = (
            self.log_alpha.expand(batch_size, 1, self.num_labels)
            .clone()
            .fill_(-10000.0)
        )
        ...
        for t in range(1, seq_size):
            log_alpha = (
                self.log_sum_exp_batch(self.transitions + log_alpha, axis=-1)
                + feats[:, t]
            ).unsqueeze(1)
    ...
    def _crf_decode(self, feats, label_ids):
        ...
        log_delta = (
            self.log_delta.expand(batch_size, 1, self.num_labels)
            .clone()
            .fill_(-10000.0)
        )
...
class NERLongformer(pl.LightningModule):
    ...
    def __init__(self, cfg, task):
        ...
        self.crf_layer = Transformer_CRF(
            ...
            start_label_id=self.tokenizer.cls_token_id,
        )
...
=======
        print(
            classification_report(
                val_labels.view(-1).cpu().detach(), val_logits.view(-1).cpu().detach()
            )
        )
>>>>>>> 2280aa021360518019405f97841de4d0a61642d7
    ...
    def test_epoch_end(self, outputs):
        ...
        tokens = [self.tokenizer.convert_ids_to_tokens(token) for token in test_tokens]

        print(
            classification_report(
                test_labels.view(-1).cpu().detach(), test_logits.view(-1).cpu().detach()
            )
        )
